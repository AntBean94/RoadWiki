<template>
  <div>
    ...
    <base-header class="pb-6 pb-8 pt-5 pt-md-8 bg-gradient-default">
      ...
      <!-- 카테고리선택 부분 -->
      <input type="radio" id="LargeCategory" value="LargeCategory" v-model="picked">
      <label for="LargeCategory">LargeCategory</label>
      <br>
      <input type="radio" id="MediumCategory" value="MediumCategory" v-model="picked">
      <label for="MediumCategory">MediumCategory</label>
      <br>
      <input type="radio" id="SmallCategory" value="SmallCategory" v-model="picked">
      ...
      <div v-if="picked === 'LargeCategory' ">
          ...
          <button @click="largeSubmit" class="btn" style="background-color: rgb(181, 199, 211); display: block; ">
              제출하기
          </button>
      </div>
      <!-- 중분류 -->
      <div v-else-if="picked === 'MediumCategory' ">
          ...
          <br>
          <textarea v-model="MediumExplanation" placeholder="중분류 설명"></textarea>
          <button class="btn" style="background-color: rgb(181, 199, 211); display: block; ">
              제출하기
          </button>
      </div>
      <!-- 소분류 -->
...
export default {
  ...
  created() {
      ...
      axios.get(`${this.$store.getters.getServer}/curriculum/suggest`)
        .then((res) => {
  ...
  methods: {
    ...
    MediumSubmit() {
        ...
        // axios.get(`${this.$store.getters.getServer}/curriculum/${this.Largeselected}`)
        //   .then{}
    },
  },
}
